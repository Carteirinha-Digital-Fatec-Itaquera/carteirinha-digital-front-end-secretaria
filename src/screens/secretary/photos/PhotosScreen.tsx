...
export default function PhotosScreen() {
  ...
  const [selected, setSelected] = useState<Set<string>>(new Set()); // ✅ novo
  ...
  const handleApprove = async (ra: string) => {
    ...
    if ('ok' in result) { toast.success('Foto aprovada!'); loadPhotos(); }
  ...
  const handleReject = async (ra: string) => {
    ...
    if ('ok' in result) { toast.success('Foto reprovada!'); loadPhotos(); }
    else toast.error(result.message);
  };
  ...
  const handleRejectSelected = async () => {
    await Promise.all([...selected].map(ra => approvePhoto(ra, false, rejectionReason[ra] ?? "")));
            ...
            <>
              ...
              <div className={styles.bulkActions}>
                ...
                <div className={styles.bulkActionsLeft}>
                  <ButtonComp text="Selecionar todos" onClick={selectAll} />
                ...
                {photos.map((p) => (
                  ...
                  >
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        e.stopPropagation();
                        setRejectionReason(prev => ({ ...prev, [p.ra]: e.target.value }));
                      }}
                      onClick={e => e.stopPropagation()}
                    />
